...
export default function CabinsPage() {
  ...
  useEffect(() => {
    ...
    const initialFilters: CabinsFilters = {
      capacity: urlParams.get('capacity') ? parseInt(urlParams.get('capacity')!) : undefined,
    ...
    return (
      <div className="flex justify-center items-center min-h-[50vh]">
        <div className="text-center">
          <h2 className="text-xl font-bold text-red-500 mb-2">Error Loading Cabins</h2>
          <p className="text-default-500">
            {error instanceof Error ? error.message : 'An error occurred while loading cabins'}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto py-8">
      ...
      <div className="text-center mb-8">
        <h1 className={title({ size: "lg" })}>Our Cabins</h1>
        <p className={subtitle({ class: "mt-4 max-w-2xl mx-auto" })}>
          Discover our collection of beautiful cabins, each offering unique experiences 
          in the heart of nature. From cozy retreats to spacious family accommodations.
        </p>
      </div>

      {/* Filters */}
      <CabinFilters onFiltersChange={handleFiltersChange} isLoading={isLoading} />

      {/* Loading State */}
      {isLoading && (
        <div className="flex flex-col justify-center items-center py-12 gap-4">
          <Spinner size="lg" />
          <div className="text-lg">Loading cabins...</div>
        </div>
      )}
