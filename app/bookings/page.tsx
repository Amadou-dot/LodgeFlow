...
import { useBookingHistory, useCancelBooking, useUpdateBooking } from '@/hooks/useBooking';
...
export default function BookingsPage() {
  ...
  return (
    <section className="flex flex-col items-center justify-center gap-4 py-8 md:py-10 px-4">
      ...
      <Modal isOpen={isDetailsOpen} scrollBehavior="inside" size="2xl" onClose={onDetailsClose}>
        <ModalContent>
          {(onClose: () => void) => (
            <>
              ...
              <ModalBody>
                {selectedBooking && (
                  <div className="space-y-6">
                    {/* Cabin Info */}
                    <div>
                      <h3 className="text-lg font-semibold mb-3">Cabin Information</h3>
                      <div className="flex gap-4">
                        <div className="relative w-32 h-32 flex-shrink-0 rounded-lg overflow-hidden">
                          <Image
                            alt={(selectedBooking as any).cabin?.name || 'Cabin'}
                            className="object-cover"
                            fill
                            src={(selectedBooking as any).cabin?.image || '/placeholder-cabin.jpg'}
                          />
                        </div>
                        <div>
                          <p className="font-semibold text-xl">{(selectedBooking as any).cabin?.name}</p>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-default-500 mt-2">
                        {selectedBooking.numNights} nights • {selectedBooking.numGuests} guests
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <Chip
                            color={selectedBooking.isPaid ? 'success' : 'warning'}
                            size="sm"
                            variant="flat"
                          >
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedBooking.extras.hasBreakfast && (
                            <Chip size="sm" variant="flat">Breakfast</Chip>
                          )}
                          {selectedBooking.extras.hasPets && (
                            <Chip size="sm" variant="flat">Pets</Chip>
                          )}
                          {selectedBooking.extras.hasParking && (
                            <Chip size="sm" variant="flat">Parking</Chip>
                          )}
                          {selectedBooking.extras.hasEarlyCheckIn && (
                            <Chip size="sm" variant="flat">Early Check-in</Chip>
                          )}
                          {selectedBooking.extras.hasLateCheckOut && (
                            <Chip size="sm" variant="flat">Late Check-out</Chip>
                          )}
                        </div>
                      </div>
                    )}
                    ...
                    {selectedBooking.observations && (
                      <div>
                        ...
                        <p className="text-sm text-default-600">{selectedBooking.observations}</p>
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        <ul className="list-disc list-inside space-y-1">
                          {selectedBooking.specialRequests.map((request, index) => (
                            <li key={index} className="text-sm text-default-600">{request}</li>
                          ))}
                        </ul>
                      </div>
                    )}
          ...
          {(onClose: () => void) => (
            <>
              ...
              <ModalBody>
                ...
                <p className="text-sm text-default-500 mt-2">
                  This action cannot be undone. Please review our cancellation policy for any applicable fees.
                </p>
              </ModalBody>
              <ModalFooter>
                <Button variant="light" onPress={onClose}>
      ...
      {/* Edit Booking Modal */}
      <Modal isOpen={isEditOpen} scrollBehavior="inside" size="2xl" onClose={onEditClose}>
        <ModalContent>
          {(onClose: () => void) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                ...
                <p className="text-sm font-normal text-default-500">
                  Update your booking details for {(selectedBooking as any)?.cabin?.name || 'your cabin'}
                </p>
              </ModalHeader>
              <ModalBody>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditFormData((prev) => ({ ...prev, checkInDate: e.target.value }))
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditFormData((prev) => ({ ...prev, checkOutDate: e.target.value }))
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    {(selectedBooking as any)?.cabin?.maxCapacity && (
                      <p className="text-xs text-default-400 mt-1">
                        Maximum capacity: {(selectedBooking as any).cabin.maxCapacity} guests
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditFormData((prev) => ({ ...prev, observations: e.target.value }))
                      }
                    />
                  </div>
                  ...
                  <div className="bg-primary/10 rounded-lg p-3">
                    <p className="text-sm text-default-700">
                      <strong>Note:</strong> Changes to dates may affect pricing. You&apos;ll be notified of any price changes before finalizing the update.
                    </p>
                  </div>
                </div>
              </ModalBody>
